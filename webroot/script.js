...
function loadLanguage(langCode) {
  if (translations[langCode]) {
    ...
    logTerminal(`Idioma: ${langCode}`, "info");
  }
}
...
// Ejecución Root unificada (Soporta KernelSU y APatch WebUI)
...
// Control de Tasa de Refresco para HyperOS
async function setRefreshRate(hz) {
  if (hz === "auto") {
    logTerminal("Restableciendo tasa de refresco a Automático...", "info");
    ...
    logTerminal(`Ajustando pantalla a ${hz}Hz...`, "info");
...
async function refreshRAM() {
  logTerminal("Consultando RAM...", "info");
  ...
  if (totalMB > 0) {
    ...
    logTerminal("Métricas de RAM actualizadas.", "success");
  }
}
...
async function refreshCPU() {
  logTerminal("Consultando CPU...", "info");
  ...
  logTerminal("Estado de CPU actualizado.", "success");
}

async function setGovernor(gov) {
  logTerminal(`Cambiando gobernador a '${gov}'...`, "info");
  ...
  logTerminal(`Gobernador fijado en '${gov}'.`, "success");
  refreshCPU();
}
...
async function toggleMiuiOptimization(enable) {
  ...
  logTerminal(`persist.sys.miui_optimization=${value}. Reinicia el dispositivo.`, "success");
}
...
async function toggleAdsDebloat(enable) {
  ...
  logTerminal(enable ? "Anuncios deshabilitados." : "Anuncios habilitados.", "success");
}
...
async function applyProfile(type) {
  if (type === 'high') {
    logTerminal("Aplicando perfil 6GB+ High Performance...", "info");
    ...
    logTerminal("Aplicando perfil 4GB- Memory Saver...", "info");
...
async function runCleanRAM() {
  logTerminal("Limpiando Caché de Memoria...", "info");
...
async function killBackgroundApps() {
  ...
  logTerminal("Aplicaciones en segundo plano finalizadas.", "success");
}

async function resetBatteryStats() {
  await execCmd("dumpsys batterystats --reset");
  logTerminal("Estadísticas de batería reiniciadas.", "success");
}

async function runTrim() {
  await execCmd("fstrim -v /data");
  logTerminal("TRIM de almacenamiento ejecutado.", "success");
}
